    ...
    const syncStore = react.useMemo(() => {
      ...
      return {
        ...
        subscribe: onStoreChange => {
          ...
          if (listen) {
            subscription = store.subscribe(query, typeof params === "undefined" ? {} : params, (err, result) => {
              if (err) {
                handleError(err);
              } else {
                snapshot = result;
...
const definePreview = config => _definePreview({
  ...config,
  importEventSourcePolyfill: () => suspendReact.suspend(() => _lazyEventSourcePolyfill(), ["@sanity/preview-kit", "@sanity/eventsource/browser"]),
